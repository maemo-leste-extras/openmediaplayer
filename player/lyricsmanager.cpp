...
LyricsManager::~LyricsManager()
{
    ...
    Q_FOREACH(QPluginLoader* loader, loadersList) {
        loader->unload();
        delete loader;
    }
}
...
void LyricsManager::populate()
{
    ...
    Q_FOREACH(QString providerName, desiredProviders) {
    ...
    Q_FOREACH(QPluginLoader *loader, loadersMap) {
        loader->unload();
        delete loader;
    }

    // Establish connections with active providers
    Q_FOREACH(AbstractLyricsProvider *provider, providersList) {
...
void LyricsManager::deleteLyrics(QString artist, QString title)
{
    ...
    if (this->title == title && this->artist == artist)
        Q_EMIT lyricsInfo(tr("Lyrics not found"));
}
...
void LyricsManager::fetchLyrics(QString artist, QString title, QString queryArtist, QString queryTitle, bool useCache)
{
    ...
    if (useCache && QFile(cacheFilePath(queryArtist, queryTitle)).exists()) {
        Q_EMIT lyricsFetched(loadLyrics(queryArtist, queryTitle));
    } else {
        Q_EMIT lyricsInfo(tr("Fetching lyrics..."));
...
void LyricsManager::queryNextProvider()
{
    ...
    if (++retry < providersList.size()) {
        ...
        retry = -1;
        Q_EMIT lyricsInfo(connectionError ? tr("There is no active Internet connection")
                                        : tr("Lyrics not found"));
    }
}
...
bool LyricsManager::clearCache()
{
    ...
    Q_FOREACH (QString artistDir, artistDirs) {
        ...
        Q_FOREACH (QString song, songFiles)
            artist.remove(song);
